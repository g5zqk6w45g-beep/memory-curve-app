...
import Streak from "@/components/Streak"; // ✅ Ajout de la flamme
...
export default function ExamPage() {
  ...
  useEffect(() => {
    if (studyingTopic) { 
      ...
      setIsFlipped(false);
      setIsMobileMenuOpen(false); // ✅ Ferme le menu si on lance une révision
    }
  }, [studyingTopic]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800 flex flex-col md:flex-row">
      ...
      `}>
        {/* Titre Sidebar (Caché sur mobile) */}
      ...
      {/* --- 3. CONTENU PRINCIPAL (Ton code d'origine est ici) --- */}
      <main className="flex-1 p-4 md:p-10 max-w-5xl overflow-y-auto min-h-screen" style={{ backgroundImage: `linear-gradient(rgba(255,255,255,0.85), rgba(255,255,255,0.85)), url('https://www.warehouse-nantes.fr/media/cache/media_upload/assets/images/bg/bg-home-GOO_d3F.webp')`, backgroundSize: 'cover', backgroundAttachment: 'fixed' }}>
        ...
        <div className="grid gap-6 mb-20"> {/* Ajout de marge en bas pour le scroll mobile */}
        ...
        {pastExams.length > 0 && (
          <div className="border-t border-gray-300 pt-8 opacity-70 mb-10">
